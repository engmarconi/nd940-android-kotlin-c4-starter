...
class RemindersActivityTest :
    ...
    @Rule
    var mActivityRule: ActivityTestRule<RemindersActivity> =
    ...
    fun init() {
        ...
        val myModule = module {
            ...
            single { RemindersLocalRepository(get()) as IRemindersRepository }
    ...
    fun addTask() = runBlocking {
        ...
        Thread.sleep(2000) //Wait map ready
        onView(withId(R.id.map)).perform(longClick())
        onView(withId(R.id.saveLocButton)).perform(longClick())
        onView(withId(R.id.reminderTitle)).perform(replaceText(title))
        onView(withId(R.id.reminderDescription)).perform(replaceText(description))
        onView(withId(R.id.saveReminder)).perform(click())

        onView(withId(R.id.reminderssRecyclerView))
            .check(matches(atPositionOnView(0, withText(title), R.id.title)));
